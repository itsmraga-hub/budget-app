...
RSpec.describe 'BudgetCategories#index', type: :feature do
  before(:each) do
    ...
    @transaction2 = Expense.create!(author: user, name: 'Groceries', amount: 100.0, budget_categories: [@budget_category1, @budget_category2])
  ...
  it 'displays all transactions dates' do
    @budget_category1.expenses.each do |expense|
      expect(page).to have_content(expense.time_created)
    end
  end

  it 'displays current category totals' do
    expect(page).to have_content(@budget_category1.total)
  end
